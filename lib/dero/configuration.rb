class Netvice::Dero::Configuration
  ...
  config_field(:workdir, -> {(defined?(Rails.root) ? Rails.root.to_s : Dir.pwd)}) do |val|
    Netvice::Dero::Kernel::Line.instance_variable_set(:@in_app_pattern, nil)
    val
  end

  # specify the http headers that if encountered, will be masked
  config_field(:masked_http_headers, ['Authorization']) do |val|
    Netvice::Dero::Processor::HttpHeaders.instance_variable_set(:@fields_re, nil)
    val
  end
  ...
  config_field(:masked_fields, DEFAULT_MASKED_FIELDS) do |val|
    ...
    val.map!(&:to_s)
    Netvice::Dero::Processor::SensitiveMasker.instance_variable_set(:@fields_re, nil)
