...
describe Netvice::Dero::Configuration do
  ...
  describe '#workdir' do
    ...
    it 'resets the in app pattern if workdir is changed' do
      ...
      expect(Netvice::Dero::Kernel::Line.in_app_pattern.to_s).to eq old_pattern
      conf.dero.workdir "newdir"
      expect(Netvice::Dero::Kernel::Line.in_app_pattern.to_s).to eq new_pattern
    end

    it 'resets the http headers regex if masked_http_headers is changed' do
      expect(Netvice::Dero::Processor::HttpHeaders.fields_re.to_s).to eq %Q{(?i-mx:Authorization)}
      conf.dero.masked_http_headers ['A', 'B']
      expect(Netvice::Dero::Processor::HttpHeaders.fields_re.to_s).to eq %Q{(?i-mx:A|B)}
    end

    it 'resets the fields regex if masked_fields is changed, and append the masked fields instead of replace' do
      expect(Netvice::Dero::Processor::SensitiveMasker.fields_re.to_s).to eq %Q{(?i-mx:authorization|password|passwd|pass|secret|ssn|social(.*)?sec)}
      conf.dero.masked_fields ['when_will_you_marry']
      expect(Netvice::Dero::Processor::SensitiveMasker.fields_re.to_s).to eq %Q{(?i-mx:authorization|password|passwd|pass|secret|ssn|social(.*)?sec|\\bwhen_will_you_marry\\b)}
    end

    it 'excludes from masked fields if it is in unmasked_fields' do
      expect(Netvice::Dero::Processor::SensitiveMasker.fields_re.to_s).to eq %Q{(?i-mx:authorization|password|passwd|pass|secret|ssn|social(.*)?sec)}
      conf.dero.unmasked_fields ['password']
      expect(Netvice::Dero::Processor::SensitiveMasker.fields_re.to_s).to eq %Q{(?i-mx:authorization|passwd|pass|secret|ssn|social(.*)?sec)}
    end
  end
end
